...
REGISTER_OP("HbPartitionByModulo")
    .Output("output: T")
    .Output("sizes: int32")
...
class PartitionByModuloOp : public OpKernel {
 public:
  PartitionByModuloOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    OP_REQUIRES(
        ...
        errors::InvalidArgument("partition_by_modulo expects a 1D vector."));
    ...
    functor::PartitionByModulo<Device, T>()(num_partitions_, input, output,
                                            sizes, indices, ctx);
  }
...
template <typename Device, typename T>
class PartitionByModuloNOp : public OpKernel {
 public:
  PartitionByModuloNOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    for (int i = 0; i < num_inputs_; ++i) {
      ...
      OP_REQUIRES(ctx, TensorShapeUtils::IsVector(input.shape()),
                  errors::InvalidArgument(
                      "partition_by_modulo_n expects 1D vector for input ", i));
      const int32 input_size = input.NumElements();
    ...
    functor::PartitionByModuloN<Device, T>()(
...
#if GOOGLE_CUDA
#define REGISTER_SHUFFLE_PARTITION_KERNEL(TYPE)           \
  REGISTER_KERNEL_BUILDER(Name("HbPartitionByModuloN")    \
                              .Device(DEVICE_GPU)         \
                              .TypeConstraint<TYPE>("T"), \
                          PartitionByModuloNOp<GPUDevice, TYPE>)
TF_CALL_SHUFFLE_PARTITION_TYPES(REGISTER_SHUFFLE_PARTITION_KERNEL);
#undef REGISTER_SHUFFLE_PARTITION_KERNEL
#endif  // GOOGLE_CUDA
