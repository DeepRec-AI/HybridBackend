...
class AlltoallvNTest(unittest.TestCase):
  def setUp(self):  # pylint: disable=invalid-name
    os.environ['CUDA_VISIBLE_DEVICES'] = '0,1'
    os.environ['NCCL_DEBUG'] = 'INFO'
    os.environ['NCCL_DEBUG_SUBSYS'] = 'ALL'
    os.environ['NCCL_LAUNCH_MODE'] = 'GROUP'
    os.environ['TF_CPP_VMODULE'] = 'nccl_alltoallv=1,nccl_alltoallv_n=1'
    ...
    try:
      with tf.Graph().as_default():
        ...
        with tf.device(devices[0]):
          ...
          out02, out_sizes02 = comm0.alltoallv_n(
        ...
        with tf.device(devices[1]):
          ...
          out13, out_sizes13 = comm1.alltoallv_n(
  ...
  def test_alltoallv_n_grad(self):
    hb.context.options.update(comm_pubsub_device='')

    devices = ['/gpu:0', '/gpu:1']
    comm_id = 'alltoallv_n_grad_test'
    server = hb.train.Server({'localhost': ['localhost:0']})
  ...
  def test_alltoallv_n_grad_half_prec(self):
    hb.context.options.update(comm_pubsub_device='')
