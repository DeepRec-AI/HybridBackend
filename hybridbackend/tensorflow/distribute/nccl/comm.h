...
#ifndef HYBRIDBACKEND_TENSORFLOW_DISTRIBUTE_NCCL_COMM_H_
#define HYBRIDBACKEND_TENSORFLOW_DISTRIBUTE_NCCL_COMM_H_
...
class NcclComm : public ResourceBase {
 public:
  ...
  void RunAsync(const string& message, OpKernelContext* ctx,
                AsyncOpKernel::DoneCallback done, std::function<void()> func);
  ...
  Status AlltoallvN(const std::vector<Tensor>& inputs, const Tensor& host_sizes,
                    const std::vector<int64>& common_sizes,
                    std::vector<Tensor*>* outputs);

  Status AlltoallvN(const std::vector<Tensor*>& inputs,
  ...
  Status AlltoallwN(const std::vector<Tensor>& inputs, const int64 num_columns,
                    std::vector<Tensor*>* outputs);
...
#endif  // HYBRIDBACKEND_TENSORFLOW_DISTRIBUTE_NCCL_COMM_H_
