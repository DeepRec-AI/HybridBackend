...
#if GOOGLE_CUDA
class NcclReduceOp : public NcclCommAsyncOp {
 public:
  explicit NcclReduceOp(OpKernelConstruction* ctx) : NcclCommAsyncOp(ctx) {
  ...
  void ComputeAsyncWithComm(NcclComm* comm, OpKernelContext* ctx,
    ...
    comm->RunAsync(
        "NcclReduce", ctx, done, [input, output, this, comm, ctx, done]() {
          VLOG(1) << comm->DebugString() << " [" << name() << "] [Reduce]";
          OP_REQUIRES_OK_ASYNC(
              ctx, comm->Reduce(*input, reduce_op_, root_rank_, output), done);
        });
  }
...
#define REGISTER_KERNEL(TYPE)                                            \
  REGISTER_KERNEL_BUILDER(                                               \
      Name("HbNcclReduce").Device(DEVICE_GPU).TypeConstraint<TYPE>("T"), \
      NcclReduceOp);
