...
__global__ void CastNFp32ToFp16(const size_t total_max_in_size,
                                const size_t max_in_size, const float** dd_in,
                                __half** dd_out, const int32* d_size) {
...
template <>
void CastN<float, Eigen::half>::operator()(const std::vector<Tensor>& in,
  ...
  CudaLaunch(CastNFp32ToFp16, total_max_in_size, 0, d, stream,
...
template struct CastN<float, Eigen::half>;

__global__ void CastNFp16ToFp32(const size_t total_max_in_size,
                                const size_t max_in_size, const __half** dd_in,
                                float** dd_out, const int32* d_size) {
  for (int32 idx : CudaGridRangeX(total_max_in_size)) {
    const int32 s = idx / max_in_size;
    const int32 sidx = idx % max_in_size;
    if (sidx < ldg(d_size + s)) {
...
template <>
void CastN<Eigen::half, float>::operator()(const std::vector<Tensor>& in,
                                           std::vector<Tensor>* out,
                                           OpKernelContext* ctx,
                                           cudaStream_t* stream) {
  const int64 num_inputs = in.size();
  ...
  auto total_max_in_size = num_inputs * max_in_size;
  CudaLaunch(CastNFp16ToFp32, total_max_in_size, 0, d, stream,
             total_max_in_size, max_in_size, dd_in, dd_out, d_size);
}
...
template struct CastN<Eigen::half, float>;
