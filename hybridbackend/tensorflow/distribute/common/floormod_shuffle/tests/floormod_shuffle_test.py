  ...
  def test_cpu(self):
    ...
    with tf.Graph().as_default():
      with tf.device('/cpu:0'):
        xi = tf.constant(x)
        y_list, idx_list = hb.distribute.floormod_partition(xi, num_partitions)
        y, ysizes, idx = hb.distribute.floormod_shuffle(xi, num_partitions)
  ...
  def test_gpu_group(self):
    ...
    with tf.Graph().as_default():
      with tf.device('/gpu:0'):
        ...
        y, ysizes, idx = hb.distribute.floormod_shuffle_n(
          xt, num_partitions)
        with hb.train.monitored_session() as sess:
