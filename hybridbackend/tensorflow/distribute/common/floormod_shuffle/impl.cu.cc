...
#include "hybridbackend/tensorflow/distribute/common/floormod_shuffle/functors.h"
...
template <typename T>
__global__ void FloorModShuffleComputeSizes(const int32 num_partitions,
...
template <typename T>
__global__ void FloorModShufflePopulate(const int32 num_partitions,
                                        const int32 input_size,
...
template <typename T>
struct FloormodShuffle<GPUDevice, T> {
...
template struct FloormodShuffle<GPUDevice, int32>;
template struct FloormodShuffle<GPUDevice, int64>;
template struct FloormodShuffle<GPUDevice, uint32>;
template struct FloormodShuffle<GPUDevice, uint64>;

template <typename T>
__global__ void FloorModShuffleNZeroSizes(const int32 total_num_partitions,
...
template <typename T>
__global__ void FloorModShuffleNComputeSizes(const int32 total_max_inputs_size,
                                             const int32 max_inputs_size,
                                             const int32 num_partitions,
                                             const int32* d_inputs_size,
                                             const size_t* dd_inputs,
...
__global__ void FloorModShuffleNComputeOffsets(const int32 num_inputs,
                                               const int32 num_partitions,
                                               size_t* dd_outputs_sizes,
...
template <typename T>
__global__ void FloorModShuffleNPopulate(
...
template <typename T>
struct FloormodShuffleN<GPUDevice, T> {
  void operator()(const int32 num_partitions, const std::vector<Tensor>& inputs,
    ...
    auto* range = ::hybridbackend::ProfilerRange::forSynch("FloormodShuffleN");
    ...
    CudaLaunch(FloorModShuffleNZeroSizes<T>, total_num_partitions, 0, d,
    ...
    CudaLaunch(FloorModShuffleNComputeSizes<T>, total_max_inputs_size, 0, d,
    ...
    CudaLaunch(FloorModShuffleNComputeOffsets, num_nonzero_inputs, 0, d,
               nullptr, num_nonzero_inputs, num_partitions, dd_outputs_sizes,
               d_output_offsets);
    CudaLaunch(FloorModShuffleNPopulate<T>, total_max_inputs_size, 0, d,
               nullptr, total_max_inputs_size, max_inputs_size, num_partitions,
...
template struct FloormodShuffleN<GPUDevice, int32>;
template struct FloormodShuffleN<GPUDevice, int64>;
template struct FloormodShuffleN<GPUDevice, uint32>;
template struct FloormodShuffleN<GPUDevice, uint64>;
