...
def emb_op_device():
  r'''Get device of sparse ops in embedding lookup.
  '''
  return Context.get().param('emb_op_device', '', env='HB_EMB_OP_DEVICE')
...
def sparse_count_nonzero(input_tensor,
  ...
  with ops.name_scope(name, 'sparse_count_nonzero'):
    ...
    return math_ops.cast(
      _ops.sparse_count_nonzero(indices, shape, axis=axis),
      dtype=dtype)
...
def _sparse_segment_combine(
  ...
  def _wrapped(*inputs):
    ...
    '''
    ids_indices, ids_values, data_, indices_ = inputs
    ids_ = sparse_tensor.SparseTensor(ids_indices, ids_values, ids_shape)
    with ops.device(emb_op_device()), ops.name_scope(combiner):
      if combiner == 'sum':
        embeddings = _sparse_segment_sum(
          ids_, data_, indices_,
          pad=pad,
          segment_rank=segment_rank,
          name=name)
      elif combiner == 'mean':
        embeddings = _sparse_segment_mean(
          ids_, data_, indices_,
          pad=pad,
          segment_rank=segment_rank,
          name=name)
      elif combiner == 'sqrtn':
        embeddings = _sparse_segment_sqrt_n(
          ids_, data_, indices_,
          pad=pad,
          segment_rank=segment_rank,
          name=name)
      elif combiner == 'tile':
        embeddings = _sparse_segment_tile(
          ids_, data_, indices_,
          pad=pad,
          name=name)
    ...
    def grad_fn(*grads):
      ...
      '''
      with ops.device(emb_op_device()):
        return gradients.gradients(
          embeddings, inputs, grad_ys=grads)
    return embeddings, grad_fn

  return _wrapped(ids.indices, ids.values, data, indices)
