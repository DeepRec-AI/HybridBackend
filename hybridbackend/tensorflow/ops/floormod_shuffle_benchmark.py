...
r'''Benchmark for shuffle, lookup and gather.
'''
...
def shard_lookup(group_weight, group_ids, params, fused=False):
  results = []
  if fused:
    group_ids, _, group_idx = hb.math.group_floormod_shuffle(
      group_ids, params.num_partitions)
    results = []
    for i, weight in enumerate(group_weight):
      with tf.name_scope(f'input_{i}'):
        embs = tf.gather(weight, group_ids[i] % params.bucket_size)
        results.append(tf.gather(embs, group_idx[i]))
    return results
  for i, weight in enumerate(group_weight):
    with tf.name_scope(f'input_{i}'):
      ids = group_ids[i]
      ids, _, idx = hb.math.floormod_shuffle(ids, params.num_partitions)
...
def build_bench_op(params):
  ...
  with tf.device(params.device):
    weight_initializer = tf.random_uniform(
      [params.bucket_size, params.dim_size], 0, 100.0, dtype=tf.float32)
    weights = [
      tf.get_variable(
        f'weight{c}',
        use_resource=False,
  ...
  inputs = [
    ...
    for c in xrange(params.num_columns)]
  bench_ops = shard_lookup(weights, inputs, params, fused=params.fused)
