...
from hybridbackend.tensorflow.training.parallel_evaluation_hook \
    import ParallelEvaluationHook

from tensorflow.python.training import training as _training
for c in _training.__dict__.values():
  if isinstance(c, type) and issubclass(c, _training.Optimizer) and \
    c not in (_training.Optimizer, _training.SyncReplicasOptimizer):
