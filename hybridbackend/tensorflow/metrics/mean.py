...
def mean(values,
  ...
  with vs.variable_scope(name, 'mean', (values, weights)):
    ...
    metric_op = metrics_impl._safe_scalar_div(total, count, 'value') \
        if hasattr(metrics_impl, '_safe_scalar_div') else \
        metrics_impl._safe_div(total, count, 'value')

    if metrics_collections:
      ops.add_to_collections(metrics_collections, metric_op)

    # pylint: disable=protected-access
    update_op = metrics_impl._safe_scalar_div(
        update_total_op, update_count_op, 'update_op') if \
            hasattr(metrics_impl, '_safe_scalar_div') else \
            metrics_impl._safe_div(
