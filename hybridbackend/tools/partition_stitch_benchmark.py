...
def shard_lookup(weight, ids, params):
  ids_shards, idx_shards = hb.math.floormod_partition(
    ids, params.num_partitions)
  embs_shards = [
    tf.gather(weight, ids_shards[i] % params.bucket_size)
    for i in xrange(params.num_partitions)]
  return tf.dynamic_stitch(idx_shards, embs_shards)
...
def build_bench_op(params):
  ...
  inputs = [
    ...
    for c in xrange(params.num_columns)]
  bench_ops = [
    shard_lookup(weights[c], inputs[c], params)
    for c in xrange(params.num_columns)]
