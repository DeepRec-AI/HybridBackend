...
#define REGISTER_SHUFFLE_PARTITION_KERNEL(TYPE)                             \
  REGISTER_KERNEL_BUILDER(                                                  \
      Name("FloorModShuffle").Device(DEVICE_CPU).TypeConstraint<TYPE>("T"), \
...
#define REGISTER_SHUFFLE_PARTITION_KERNEL(TYPE)                             \
  REGISTER_KERNEL_BUILDER(                                                  \
      Name("FloorModShuffle").Device(DEVICE_GPU).TypeConstraint<TYPE>("T"), \
      FloorModShuffleOp<GPUDevice, TYPE>)
TF_CALL_SHUFFLE_PARTITION_TYPES(REGISTER_SHUFFLE_PARTITION_KERNEL);
#undef REGISTER_SHUFFLE_PARTITION_KERNEL
#endif  // GOOGLE_CUDA

REGISTER_OP("GroupFloorModShuffle")
    .Output("outputs: N * T")
...
template <typename Device, typename T>
class GroupFloorModShuffleOp : public OpKernel {
 public:
  GroupFloorModShuffleOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    for (int i = 0; i < num_inputs_; ++i) {
      ...
      OP_REQUIRES(
          ...
          errors::InvalidArgument(
              "group_floormod_shuffle expects 1D vector for input ", i));
      const int32 input_size = input.NumElements();
    ...
    functor::GroupFloorModShuffle<Device, T>()(
...
#if GOOGLE_CUDA
#define REGISTER_SHUFFLE_PARTITION_KERNEL(TYPE)           \
  REGISTER_KERNEL_BUILDER(Name("GroupFloorModShuffle")    \
                              .Device(DEVICE_GPU)         \
                              .TypeConstraint<TYPE>("T"), \
                          GroupFloorModShuffleOp<GPUDevice, TYPE>)
TF_CALL_SHUFFLE_PARTITION_TYPES(REGISTER_SHUFFLE_PARTITION_KERNEL);
#undef REGISTER_SHUFFLE_PARTITION_KERNEL
#endif  // GOOGLE_CUDA
