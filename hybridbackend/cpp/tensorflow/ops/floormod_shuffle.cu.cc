...
#include "hybridbackend/cpp/common/atomic.cu.h"
...
template <typename T>
__global__ void GroupFloorModShuffleZeroSizes(const int32 total_num_partitions,
...
template <typename T>
__global__ void GroupFloorModShuffleComputeSizes(
...
__global__ void GroupFloorModShuffleComputeOffsets(const int32 num_inputs,
                                                   const int32 num_partitions,
...
template <typename T>
__global__ void GroupFloorModShufflePopulate(
    const int32 total_max_inputs_size, const int32 max_inputs_size,
    const int32 num_partitions, const int32* d_inputs_size,
...
template <typename T>
struct GroupFloorModShuffle<GPUDevice, T> {
  void operator()(const int32 num_partitions, const std::vector<Tensor>& inputs,
    ...
    CudaLaunch(GroupFloorModShuffleZeroSizes<T>, total_num_partitions, 0, d,
    ...
    CudaLaunch(GroupFloorModShuffleComputeSizes<T>, total_max_inputs_size, 0, d,
    ...
    CudaLaunch(GroupFloorModShuffleComputeOffsets, num_nonzero_inputs, 0, d,
               nullptr, num_nonzero_inputs, num_partitions, dd_outputs_sizes,
               d_output_offsets);
    CudaLaunch(GroupFloorModShufflePopulate<T>, total_max_inputs_size, 0, d,
               nullptr, total_max_inputs_size, max_inputs_size, num_partitions,
    ...
    ctx->device()->tensorflow_gpu_device_info()->event_mgr->ThenExecute(
        ...
        [ref_hd_all_outputs_sizes_t, ref_h_inputs_size_t, ref_d_inputs_size_t,
         ref_hd_ptrs_t, ref_dd_ptrs_t, ref_d_offsets_buffer_t]() {
          ref_hd_all_outputs_sizes_t.Unref();
...
template struct GroupFloorModShuffle<GPUDevice, int32>;
template struct GroupFloorModShuffle<GPUDevice, int64>;
template struct GroupFloorModShuffle<GPUDevice, uint32>;
template struct GroupFloorModShuffle<GPUDevice, uint64>;
