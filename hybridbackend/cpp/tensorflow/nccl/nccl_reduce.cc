...
REGISTER_OP("ReduceWithNcclComm")
    .Output("output: T")
...
#if GOOGLE_CUDA
class ReduceWithNcclCommOp : public NcclCommAsyncOp {
 public:
  explicit ReduceWithNcclCommOp(OpKernelConstruction* ctx)
      : NcclCommAsyncOp(ctx) {
...
#define REGISTER_KERNEL(TYPE)                                                  \
  REGISTER_KERNEL_BUILDER(                                                     \
      Name("ReduceWithNcclComm").Device(DEVICE_GPU).TypeConstraint<TYPE>("T"), \
      ReduceWithNcclCommOp);
