...
Status NcclComm::Initialize(int size, int rank, const string& shared_name,
  ...
  if (!TF_PREDICT_TRUE(0 <= rank && rank < size)) {
    return errors::InvalidArgument(
        strings::StrCat("NCCL rank ", rank, " or size ", size, " is invalid."));
  }
  ...
  debug_string_ = strings::StrCat("NcclComm(name=", shared_name,
                                  ", size=", size, ", rank=", rank, ")");
...
Status NcclComm::Create(const string& id) {
  if (!TF_PREDICT_TRUE(id.size() == NCCL_UNIQUE_ID_BYTES)) {
    return errors::InvalidArgument(
        strings::StrCat("NCCL ID ", id.c_str(), " is invalid."));
  }
...
void NcclComm::RunAsync(std::function<void()> func, OpKernelContext* ctx,
  ...
  thread_pool_->Schedule([device_id, inputs_ready, func, this, ctx, done]() {
    ...
    ctx->device()->tensorflow_gpu_device_info()->event_mgr->ThenExecute(
        ctx->op_device_context()->stream(), [outputs_ready, ctx, done]() {
...
Status NcclComm::Reduce(Tensor* output, const Tensor& input,
...
Status NcclComm::ReduceScatter(Tensor* output, const Tensor& input,
...
Status NcclComm::Allreduce(Tensor* output, const Tensor& input,
                           const ncclRedOp_t reduce_op) {
  const void* sendbuf = input.tensor_data().data();
  void* recvbuf = const_cast<char*>(output->tensor_data().data());
  const size_t count = input.NumElements();
...
Status NcclComm::Broadcast(Tensor* output, const Tensor& input,
                           const int root_rank) {
  const void* sendbuf = input.tensor_data().data();
  void* recvbuf = const_cast<char*>(output->tensor_data().data());
  const size_t count = input.NumElements();
...
Status NcclComm::Allgather(Tensor* output, const Tensor& input) {
  const void* sendbuf = input.tensor_data().data();
  void* recvbuf = const_cast<char*>(output->tensor_data().data());
  const size_t count = input.NumElements();
  ncclDataType_t nccl_dtype(ncclFloat);
...
Status NcclComm::Allgatherv(Tensor* output, const Tensor& input,
                            const Tensor& host_sizes) {
  const void* sendbuf = input.tensor_data().data();
  char* recvbuf = const_cast<char*>(output->tensor_data().data());
  const int64* sendcounts = host_sizes.flat<int64>().data();
  ncclDataType_t nccl_dtype(ncclFloat);
  TF_RETURN_IF_ERROR(EnumToNcclEnum(input.dtype(), &nccl_dtype));
...
Status NcclComm::Alltoall(Tensor* output, const Tensor& input) {
...
Status NcclComm::Alltoallv(Tensor* output, const Tensor& input,
                           const Tensor& host_sizes) {
  const char* sendbuf = input.tensor_data().data();
  char* recvbuf = const_cast<char*>(output->tensor_data().data());
  const int64* sendcounts = host_sizes.flat<int64>().data();
  ...
  ncclGroupStart();
  for (int i = 0; i < size_; ++i) {
    const size_t sendsize = sendcounts[size_ * rank_ + i];
    const size_t recvsize = sendcounts[size_ * i + rank_];
...
Status NcclComm::Alltoallw(std::vector<Tensor*>* outputs,
...
Status NcclComm::GroupAlltoallw(std::vector<Tensor*>* outputs,
                                const std::vector<Tensor>& inputs,
                                const int64 group_size) {
  if (TF_PREDICT_FALSE(inputs.size() != group_size * size_ ||
                       outputs->size() != group_size * size_)) {
    return errors::InvalidArgument(
        "Size of send_tensors and recv_tensors must be group_size times of "
  ...
  for (int i = 0; i < inputs.size(); ++i) {
    ...
    TF_RETURN_IF_ERROR(
        NcclErrorToStatus(ncclSend(sendbufs[i], sendcounts[i], senddtypes[i],
                                   (i % size_), comm_, *stream_)));
    TF_RETURN_IF_ERROR(
        NcclErrorToStatus(ncclRecv(recvbufs[i], recvcounts[i], recvdtypes[i],
                                   (i % size_), comm_, *stream_)));
  }
  ncclGroupEnd();
  return Status::OK();
#else
  return errors::Unimplemented("GroupAlltoallw not supported in NCCL < 2.7");
#endif
}
