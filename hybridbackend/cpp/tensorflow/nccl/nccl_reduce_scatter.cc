...
REGISTER_OP("ReduceScatterWithNcclComm")
    .Output("output: T")
...
#if GOOGLE_CUDA
class ReduceScatterWithNcclCommOp : public NcclCommAsyncOp {
 public:
  explicit ReduceScatterWithNcclCommOp(OpKernelConstruction* ctx)
      : NcclCommAsyncOp(ctx) {
    int reduce_op;
    ...
    comm->RunAsync(
        [input, output, this, comm, ctx, done]() {
          ...
          OP_REQUIRES_OK_ASYNC(
              ctx, comm->ReduceScatter(output, *input, reduce_op_), done);
        },
        ctx, done);
  }
...
#define REGISTER_KERNEL(TYPE)                               \
  REGISTER_KERNEL_BUILDER(Name("ReduceScatterWithNcclComm") \
                              .Device(DEVICE_GPU)           \
                              .TypeConstraint<TYPE>("T"),   \
                          ReduceScatterWithNcclCommOp);
