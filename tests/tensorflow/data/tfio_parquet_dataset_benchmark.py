...
r'''tensorflow-io parquet benchmark.
'''
...
from tensorflow_io.parquet import ParquetDataset
...
def dtype_for_field(_):
  return dtypes.int64


def benchmark(params):
  schema = ParquetFile(params.filename).metadata.schema.to_arrow_schema()
  with ops.Graph().as_default():
    ds = ParquetDataset(
        params.filename,
        schema.names,
        tuple(dtype_for_field(f) for f in schema.names))
